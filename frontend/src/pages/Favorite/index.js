...
const Favorite = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      try {
        const response = await axios.get('http://192.168.0.11:3333/favorite');
  ...
  const handleRemoveFavorite = async (productId) => {
    try {
      await axios.delete(`http://192.168.0.11:3333/favorite/${productId}`);
...
const ProductDetails = ({ itemId }) => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const response = await axios.get(`http://192.168.0.11:3333/products/${itemId}`);
