...
export default function FinishedProduct({ navigation }) {
  ...
  useEffect(() => {
    const fetchFinishedOrders = async () => {
      try {
        const response = await fetch('http://192.168.0.11:3333/finalizeOrder');
            ...
            result.map(async (order) => {
              const updatedProducts = await Promise.all(
                order.products.map(async (product) => {
                  if (product.product) {
                    const productDetailsResponse = await fetch(`http://192.168.0.11:3333/products/${product.product._id}`);
